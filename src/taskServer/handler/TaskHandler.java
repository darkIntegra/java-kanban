...
public class TaskHandler extends BaseHttpHandler {
    ...
    protected void handleGet(String query, HttpExchange exchange) throws IOException {
        try {
            ...
            if (query == null || query.isEmpty()) {
                ...
                Task task = taskManager.getTaskById(taskId); // Может выбросить NotFoundException
            ...
            handleErrorResponse(e, 404, exchange); // Обработка случая, когда задача не найдена
        }
    }
    ...
    protected void handlePost(HttpExchange exchange) throws IOException {
        try {
            ...
            if (newTask.getId() == 0 || newTask.getId() <= 0) {
                // Создаем новую задачу через TaskManager
                taskManager.createTask(newTask);
                response = "Задача успешно добавлена.";
            } else {
                // Проверяем, существует ли задача с указанным ID
                if (!taskManager.containsTask(newTask.getId())) {
                    sendText(exchange, "Задача с ID " + newTask.getId() + " не существует.", 404);
                    return;
                }
            ...
            // Возвращаем код 201 для обоих случаев (создание и обновление)
    ...
    protected void handlePut(String query, HttpExchange exchange) throws IOException {
        try {
            ...
            if (!taskManager.containsTask(taskId)) {
                throw new NotFoundException("Задача с ID " + taskId + " не существует.");
            }
    ...
    protected void handleErrorResponse(Exception e, int statusCode, HttpExchange exchange) throws IOException {
        String errorMessage = e.getMessage(); // Получаем сообщение об ошибке
        sendText(exchange, errorMessage, statusCode); // Отправляем ошибку с указанным кодом
    }
